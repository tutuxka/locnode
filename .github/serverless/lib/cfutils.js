...
class WranglerCmd {
  ...
  publishProject() {
    ...
    const wranglerCmd = `wrangler pages publish dist --project-name ${projectName} --branch ${branch}`;
  ...
  createAppDbTables() {
    ...
    const wranglerCmd = `wrangler d1 execute ${dbName} --file dist/db.sql --remote`;
    console.log(wranglerCmd);
    return this._getCmd(wranglerCmd);
  }
